...
class JoinReorderSuite extends JoinReorderPlanTestBase with StatsEstimationTestBase {
  ...
  test("SPARK-26352: join reordering should not change the order of attributes") {
    ...
    val tab1 = LocalRelation(Symbol("x").int, Symbol("y").int)
    val tab2 = LocalRelation(Symbol("i").int, Symbol("j").int)
    val tab3 = LocalRelation(Symbol("a").int, Symbol("b").int)
    val original =
      tab1.join(tab2, Cross)
          .join(tab3, Inner, Some(Symbol("a") === Symbol("x") && Symbol("b") === Symbol("i")))
    val expected =
      tab1.join(tab3, Inner, Some(Symbol("a") === Symbol("x")))
          .join(tab2, Cross, Some(Symbol("b") === Symbol("i")))
