...
class TransposeWindowSuite extends PlanTest {
  ...
  val testRelation =
    LocalRelation(Symbol("a").string, Symbol("b").string, Symbol("c").int, Symbol("d").string)
  ...
  test("don't transpose two adjacent windows with intersection of partition and output set") {
    val query = testRelation
      .window(Seq((Symbol("a") + Symbol("b")).as(Symbol("e")), sum(c).as(Symbol("sum_a_2"))),
        partitionSpec3, Seq.empty)
      .window(Seq(sum(c).as(Symbol("sum_a_1"))), Seq(a, Symbol("e")), Seq.empty)
