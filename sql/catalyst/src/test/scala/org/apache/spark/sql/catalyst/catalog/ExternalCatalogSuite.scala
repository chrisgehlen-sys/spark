...
abstract class ExternalCatalogSuite extends SparkFunSuite with BeforeAndAfterEach {
  ...
  test("list partitions by filter") {
    ...
    checkAnswer(tbl2, Seq(Symbol("a").int <= 1), Set(part1))
    checkAnswer(tbl2, Seq(Symbol("a").int === 2), Set.empty)
    checkAnswer(tbl2, Seq(In(Symbol("a").int * 10, Seq(30))), Set(part2))
    checkAnswer(tbl2, Seq(Not(In(Symbol("a").int, Seq(4)))), Set(part1, part2))
    checkAnswer(tbl2, Seq(Symbol("a").int === 1, Symbol("b").string === "2"), Set(part1))
    checkAnswer(tbl2, Seq(Symbol("a").int === 1 && Symbol("b").string === "2"), Set(part1))
    checkAnswer(tbl2, Seq(Symbol("a").int === 1, Symbol("b").string === "x"), Set.empty)
    checkAnswer(tbl2, Seq(Symbol("a").int === 1 || Symbol("b").string === "x"), Set(part1))

    intercept[AnalysisException] {
      try {
        checkAnswer(tbl2, Seq(Symbol("a").int > 0 && Symbol("col1").int > 0), Set.empty)
      } catch {
