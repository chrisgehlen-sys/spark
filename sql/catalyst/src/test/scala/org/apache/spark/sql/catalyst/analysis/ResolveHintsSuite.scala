...
class ResolveHintsSuite extends AnalysisTest {
  ...
  test("do not traverse past existing broadcast hints") {
    checkAnalysis(
      ...
      ResolvedHint(testRelation.where(Symbol("a") > 1),
        HintInfo(strategy = Some(BROADCAST))).analyze,
      caseSensitive = false)
  }
  ...
  test("do not traverse past subquery alias") {
    checkAnalysis(
      UnresolvedHint("MAPJOIN", Seq("table"), table("table")
        .where(Symbol("a") > 1).subquery(Symbol("tableAlias"))),
  ...
  test("should work for CTE") {
    checkAnalysis(
      ...
      ResolvedHint(testRelation.where(Symbol("a") > 1)
        .select(Symbol("a")), HintInfo(strategy = Some(BROADCAST)))
