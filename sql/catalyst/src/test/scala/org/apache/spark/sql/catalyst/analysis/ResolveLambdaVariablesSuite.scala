...
class ResolveLambdaVariablesSuite extends PlanTest {
  ...
  private val key = Symbol("key").int
  private val values1 = Symbol("values1").array(IntegerType)
  private val values2 = Symbol("values2").array(ArrayType(ArrayType(IntegerType)))
  ...
  test("resolution - nested") {
    val in = ArrayTransform(values2, LambdaFunction(
      ArrayTransform(lv(Symbol("x")),
        LambdaFunction(lv(Symbol("x")) + 1, lv(Symbol("x")) :: Nil)), lv(Symbol("x")) :: Nil))
