...
class ComplexTypeSuite extends SparkFunSuite with ExpressionEvalHelper {
  ...
  test("test dsl for complex type") {
    ...
    checkEvaluation(
      quickResolve(Symbol("c").map(MapType(StringType, StringType)).at(0).getItem("a")),
      "b", create_row(Map("a" -> "b")))
    checkEvaluation(quickResolve(Symbol("c").array(StringType).at(0).getItem(1)),
      "b", create_row(Seq("a", "b")))
    checkEvaluation(quickResolve(Symbol("c").struct(Symbol("a").int).at(0).getField("a")),
