...
class StarJoinReorderSuite extends JoinReorderPlanTestBase with StatsEstimationTestBase {
  ...
  private val d3_ns = LocalRelation(Symbol("d3_fk1").int, Symbol("d3_c2").int, Symbol("d3_pk1").int,
    Symbol("d3_c4").int)
  ...
  private val subq = d3.select(sum(Symbol("d3_fk1")).as(Symbol("col")))
  ...
  test("Test 6: Join with complex plans") {
    ...
    val expected =
      d3.select(Symbol("d3_fk1")).select(sum(Symbol("d3_fk1")).as(Symbol("col")))
