...
class EliminateSortsSuite extends AnalysisTest {
  ...
  val testRelationB = LocalRelation(Symbol("d").int)
  ...
  test("Partial order-by clauses contain no-op SortOrder") {
    withSQLConf(CASE_SENSITIVE.key -> "true", ORDER_BY_ORDINAL.key -> "false") {
      ...
      val query = x.orderBy(SortOrder(3, Ascending), Symbol("a").asc)
      val optimized = Optimize.execute(analyzer.execute(query))
      val correctAnswer = analyzer.execute(x.orderBy(Symbol("a").asc))
  ...
  test("Remove no-op alias") {
    ...
    val query = x.select(Symbol("a").as(Symbol("x")),
      Year(CurrentDate()).as(Symbol("y")), Symbol("b"))
      .orderBy(Symbol("x").asc, Symbol("y").asc, Symbol("b").desc)
    val optimized = Optimize.execute(analyzer.execute(query))
    val correctAnswer = analyzer.execute(
      x.select(Symbol("a").as(Symbol("x")), Year(CurrentDate()).as(Symbol("y")), Symbol("b"))
        .orderBy(Symbol("x").asc, Symbol("b").desc))
  ...
  test("SPARK-33183: remove redundant sort by") {
    val orderedPlan = testRelation
      .select(Symbol("a"), Symbol("b")).orderBy(Symbol("a").asc, Symbol("b").desc_nullsFirst)
    val unnecessaryReordered = orderedPlan.limit(2)
      .select(Symbol("a")).sortBy(Symbol("a").asc, Symbol("b").desc_nullsFirst)
    val optimized = Optimize.execute(unnecessaryReordered.analyze)
    val correctAnswer = orderedPlan.limit(2).select(Symbol("a")).analyze
    comparePlans(optimized, correctAnswer)
  }

  test("SPARK-33183: remove all redundant local sorts") {
    val orderedPlan = testRelation.sortBy(Symbol("a").asc).orderBy(Symbol("a").asc)
      .sortBy(Symbol("a").asc)
    val optimized = Optimize.execute(orderedPlan.analyze)
    val correctAnswer = testRelation.orderBy(Symbol("a").asc).analyze
    comparePlans(optimized, correctAnswer)
  }
  ...
  test("SPARK-33183: should not remove global sort") {
    val orderedPlan = testRelation
      .select(Symbol("a"), Symbol("b")).orderBy(Symbol("a").asc, Symbol("b").desc_nullsFirst)
    val reordered = orderedPlan.limit(2)
      .select(Symbol("a")).orderBy(Symbol("a").asc, Symbol("b").desc_nullsFirst)
  ...
  test("do not remove sort if the order is different") {
    val orderedPlan = testRelation
      .select(Symbol("a"), Symbol("b")).orderBy(Symbol("a").asc, Symbol("b").desc_nullsFirst)
    val reorderedDifferently = orderedPlan.limit(2)
      .select(Symbol("a")).orderBy(Symbol("a").asc, Symbol("b").desc)
  ...
  test("SPARK-33183: remove top level local sort with filter operators") {
    val orderedPlan = testRelation
      .select(Symbol("a"), Symbol("b")).orderBy(Symbol("a").asc, Symbol("b").desc)
    val filteredAndReordered =
      orderedPlan.where(Symbol("a") > Literal(10)).sortBy(Symbol("a").asc, Symbol("b").desc)
    val optimized = Optimize.execute(filteredAndReordered.analyze)
    val correctAnswer = orderedPlan.where(Symbol("a") > Literal(10)).analyze
    comparePlans(optimized, correctAnswer)
  }

  test("SPARK-33183: keep top level global sort with filter operators") {
    val projectPlan = testRelation.select(Symbol("a"), Symbol("b"))
    val orderedPlan = projectPlan.orderBy(Symbol("a").asc, Symbol("b").desc)
    val filteredAndReordered =
      orderedPlan.where(Symbol("a") > Literal(10)).orderBy(Symbol("a").asc, Symbol("b").desc)
    val optimized = Optimize.execute(filteredAndReordered.analyze)
    val correctAnswer = projectPlan.where(Symbol("a") > Literal(10))
      .orderBy(Symbol("a").asc, Symbol("b").desc).analyze
    comparePlans(optimized, correctAnswer)
  }

  test("SPARK-33183: limits should not affect order for local sort") {
    val orderedPlan =
      testRelation.select(Symbol("a"), Symbol("b")).orderBy(Symbol("a").asc, Symbol("b").desc)
    val filteredAndReordered =
      orderedPlan.limit(Literal(10)).sortBy(Symbol("a").asc, Symbol("b").desc)
    val optimized = Optimize.execute(filteredAndReordered.analyze)
  ...
  test("SPARK-33183: should not remove global sort with limit operators") {
    val orderedPlan =
      testRelation.select(Symbol("a"), Symbol("b")).orderBy(Symbol("a").asc, Symbol("b").desc)
    val filteredAndReordered =
      orderedPlan.limit(Literal(10)).orderBy(Symbol("a").asc, Symbol("b").desc)
    val optimized = Optimize.execute(filteredAndReordered.analyze)
  ...
  test("different sorts are not simplified if limit is in between") {
    val orderedPlan = testRelation.select(Symbol("a"), Symbol("b"))
      .orderBy(Symbol("b").desc).limit(Literal(10))
      .orderBy(Symbol("a").asc)
    val optimized = Optimize.execute(orderedPlan.analyze)
  ...
  test("SPARK-33183: should not remove global sort with range operator") {
    ...
    val orderedPlan = inputPlan.orderBy(Symbol("id").asc)
    val optimized = Optimize.execute(orderedPlan.analyze)
    val correctAnswer = orderedPlan.analyze
    comparePlans(optimized, correctAnswer)
  ...
  test("SPARK-33183: remove local sort with range operator") {
    val inputPlan = Range(1L, 1000L, 1, 10)
    val orderedPlan = inputPlan.sortBy(Symbol("id").asc)
    val optimized = Optimize.execute(orderedPlan.analyze)
  ...
  test("sort should not be removed when there is a node which doesn't guarantee any order") {
    val orderedPlan = testRelation.select(Symbol("a"), Symbol("b"))
    val groupedAndResorted =
      orderedPlan.groupBy(Symbol("a"))(sum(Symbol("a"))).orderBy(Symbol("a").asc)
  ...
  test("remove two consecutive sorts") {
    val orderedTwice = testRelation.orderBy(Symbol("a").asc).orderBy(Symbol("b").desc)
    val optimized = Optimize.execute(orderedTwice.analyze)
    val correctAnswer = testRelation.orderBy(Symbol("b").desc).analyze
    comparePlans(optimized, correctAnswer)
  }

  test("remove sorts separated by Filter/Project operators") {
    val orderedTwiceWithProject = testRelation.orderBy(Symbol("a").asc)
      .select(Symbol("b")).orderBy(Symbol("b").desc)
    val optimizedWithProject = Optimize.execute(orderedTwiceWithProject.analyze)
    val correctAnswerWithProject =
      testRelation.select(Symbol("b")).orderBy(Symbol("b").desc).analyze
    comparePlans(optimizedWithProject, correctAnswerWithProject)

    val orderedTwiceWithFilter =
      testRelation.orderBy(Symbol("a").asc).where(Symbol("b") > Literal(0))
        .orderBy(Symbol("b").desc)
    val optimizedWithFilter = Optimize.execute(orderedTwiceWithFilter.analyze)
    val correctAnswerWithFilter =
      testRelation.where(Symbol("b") > Literal(0)).orderBy(Symbol("b").desc).analyze
    comparePlans(optimizedWithFilter, correctAnswerWithFilter)

    val orderedTwiceWithBoth =
      testRelation.orderBy(Symbol("a").asc).select(Symbol("b"))
        .where(Symbol("b") > Literal(0)).orderBy(Symbol("b").desc)
    val optimizedWithBoth = Optimize.execute(orderedTwiceWithBoth.analyze)
    val correctAnswerWithBoth =
      testRelation.select(Symbol("b")).where(Symbol("b") > Literal(0))
        .orderBy(Symbol("b").desc).analyze
    comparePlans(optimizedWithBoth, correctAnswerWithBoth)

    val orderedThrice = orderedTwiceWithBoth.select((Symbol("b") + 1).as(Symbol("c")))
      .orderBy(Symbol("c").asc)
    val optimizedThrice = Optimize.execute(orderedThrice.analyze)
    val correctAnswerThrice = testRelation.select(Symbol("b")).where(Symbol("b") > Literal(0))
      .select((Symbol("b") + 1).as(Symbol("c"))).orderBy(Symbol("c").asc).analyze
  ...
  test("remove orderBy in groupBy clause with order irrelevant aggs") {
    Seq(
      ...
      val projectPlan = testRelation.select(Symbol("a"), Symbol("b"))
      val unnecessaryOrderByPlan = projectPlan.orderBy(Symbol("a").asc, Symbol("b").desc)
      val groupByPlan = unnecessaryOrderByPlan.groupBy(Symbol("a"))(agg(Symbol("b")))
      val optimized = Optimize.execute(groupByPlan.analyze)
      val correctAnswer = projectPlan.groupBy(Symbol("a"))(agg(Symbol("b"))).analyze
      comparePlans(optimized, correctAnswer)
    })
  }
  ...
  test("remove orderBy in groupBy clause with sum aggs") {
    val projectPlan = testRelation.select(Symbol("a"), Symbol("b"))
    val unnecessaryOrderByPlan = projectPlan.orderBy(Symbol("a").asc, Symbol("b").desc)
    val groupByPlan = unnecessaryOrderByPlan.groupBy(Symbol("a"))(sum(Symbol("a")) + 10 as "sum")
    val optimized = Optimize.execute(groupByPlan.analyze)
    val correctAnswer = projectPlan.groupBy(Symbol("a"))(sum(Symbol("a")) + 10 as "sum").analyze
    comparePlans(optimized, correctAnswer)
  }

  test("should not remove orderBy in groupBy clause with first aggs") {
    val projectPlan = testRelation.select(Symbol("a"), Symbol("b"))
    val orderByPlan = projectPlan.orderBy(Symbol("a").asc, Symbol("b").desc)
    val groupByPlan = orderByPlan.groupBy(Symbol("a"))(first(Symbol("a")))
    val optimized = Optimize.execute(groupByPlan.analyze)
  ...
  test("should not remove orderBy in groupBy clause with first and count aggs") {
    val projectPlan = testRelation.select(Symbol("a"), Symbol("b"))
    val orderByPlan = projectPlan.orderBy(Symbol("a").asc, Symbol("b").desc)
    val groupByPlan = orderByPlan.groupBy(Symbol("a"))(first(Symbol("a")), count(1))
    val optimized = Optimize.execute(groupByPlan.analyze)
    val correctAnswer = groupByPlan.analyze
    comparePlans(optimized, correctAnswer)
  }
  ...
  test("should not remove orderBy in groupBy clause with PythonUDF as aggs") {
    ...
    val projectPlan = testRelation.select(Symbol("a"), Symbol("b"))
    val orderByPlan = projectPlan.orderBy(Symbol("a").asc, Symbol("b").desc)
    val groupByPlan = orderByPlan.groupBy(Symbol("a"))(pythonUdf)
    val optimized = Optimize.execute(groupByPlan.analyze)
    val correctAnswer = groupByPlan.analyze
    comparePlans(optimized, correctAnswer)
  }
  ...
  test("should not remove orderBy in groupBy clause with ScalaUDF as aggs") {
    val scalaUdf = ScalaUDF((s: Int) => s, IntegerType, Symbol("a") :: Nil,
      Option(ExpressionEncoder[Int]()) :: Nil)
    val projectPlan = testRelation.select(Symbol("a"), Symbol("b"))
    val orderByPlan = projectPlan.orderBy(Symbol("a").asc, Symbol("b").desc)
    val groupByPlan = orderByPlan.groupBy(Symbol("a"))(scalaUdf)
    val optimized = Optimize.execute(groupByPlan.analyze)
    val correctAnswer = groupByPlan.analyze
    comparePlans(optimized, correctAnswer)
  }
  ...
  test("should not remove orderBy with limit in groupBy clause") {
    val projectPlan = testRelation.select(Symbol("a"), Symbol("b"))
    val orderByPlan = projectPlan.orderBy(Symbol("a").asc, Symbol("b").desc).limit(10)
    val groupByPlan = orderByPlan.groupBy(Symbol("a"))(count(1))
    val optimized = Optimize.execute(groupByPlan.analyze)
    val correctAnswer = groupByPlan.analyze
    comparePlans(optimized, correctAnswer)
  }
  ...
  test("remove orderBy in join clause") {
    val projectPlan = testRelation.select(Symbol("a"), Symbol("b"))
    val unnecessaryOrderByPlan = projectPlan.orderBy(Symbol("a").asc, Symbol("b").desc)
    val projectPlanB = testRelationB.select(Symbol("d"))
    val joinPlan = unnecessaryOrderByPlan.join(projectPlanB).select(Symbol("a"), Symbol("d"))
    val optimized = Optimize.execute(joinPlan.analyze)
    val correctAnswer = projectPlan.join(projectPlanB).select(Symbol("a"), Symbol("d")).analyze
    comparePlans(optimized, correctAnswer)
  }
  ...
  test("should not remove orderBy with limit in join clause") {
    ...
    val orderByPlan = projectPlan.orderBy(Symbol("a").asc, Symbol("b").desc).limit(10)
    val projectPlanB = testRelationB.select(Symbol("d"))
    val joinPlan = orderByPlan.join(projectPlanB).select(Symbol("a"), Symbol("d"))
    val optimized = Optimize.execute(joinPlan.analyze)
  ...
  test("SPARK-32318: should not remove orderBy in distribute statement") {
    val projectPlan = testRelation.select(Symbol("a"), Symbol("b"))
    val orderByPlan = projectPlan.orderBy(Symbol("b").desc)
    val distributedPlan = orderByPlan.distribute(Symbol("a"))(1)
  ...
  test("SPARK-33183: remove consecutive global sorts with the same ordering") {
    Seq(
      (testRelation.orderBy(Symbol("a").asc).orderBy(Symbol("a").asc),
        testRelation.orderBy(Symbol("a").asc)),
      (testRelation.orderBy(Symbol("a").asc, Symbol("b").desc).orderBy(Symbol("a").asc),
        testRelation.orderBy(Symbol("a").asc))
  ...
  test("SPARK-33183: remove consecutive local sorts with the same ordering") {
    val orderedPlan = testRelation.sortBy(Symbol("a").asc).sortBy(Symbol("a").asc)
      .sortBy(Symbol("a").asc)
    val optimized = Optimize.execute(orderedPlan.analyze)
    val correctAnswer = testRelation.sortBy(Symbol("a").asc).analyze
    comparePlans(optimized, correctAnswer)
  }

  test("SPARK-33183: remove consecutive local sorts with different ordering") {
    val orderedPlan = testRelation.sortBy(Symbol("b").asc).sortBy(Symbol("a").desc)
      .sortBy(Symbol("a").asc)
    val optimized = Optimize.execute(orderedPlan.analyze)
    val correctAnswer = testRelation.sortBy(Symbol("a").asc).analyze
    comparePlans(optimized, correctAnswer)
  ...
  test("SPARK-33183: should keep global sort when child is a local sort with the same ordering") {
    val correctAnswer = testRelation.orderBy(Symbol("a").asc).analyze
    Seq(
      testRelation.sortBy(Symbol("a").asc).orderBy(Symbol("a").asc),
      testRelation.orderBy(Symbol("a").asc).sortBy(Symbol("a").asc).orderBy(Symbol("a").asc)
    ).foreach { ordered =>
      val optimized = Optimize.execute(ordered.analyze)
      comparePlans(optimized, correctAnswer)
    }
