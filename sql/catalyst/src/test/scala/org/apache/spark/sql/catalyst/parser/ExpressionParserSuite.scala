...
class ExpressionParserSuite extends AnalysisTest {
  ...
  test("in sub-query") {
    ...
    assertEqual(
      ...
      InSubquery(Seq(Symbol("a"), Symbol("b"), Symbol("c")), ListQuery(table("g")
        .select(Symbol("d"), Symbol("e"), Symbol("f")))))
  ...
  test("(NOT) LIKE (ANY | SOME | ALL) expressions") {
    Seq("any", "some").foreach { quantifier =>
      assertEqual(s"a like $quantifier ('foo%', 'b%')",
        (Symbol("a") like "foo%") || (Symbol("a") like "b%"))
      assertEqual(s"a not like $quantifier ('foo%', 'b%')",
        !(Symbol("a") like "foo%") || !(Symbol("a") like "b%"))
      assertEqual(s"not (a like $quantifier ('foo%', 'b%'))",
        !((Symbol("a") like "foo%") || (Symbol("a") like "b%")))
    }
    assertEqual("a like all ('foo%', 'b%')",
      (Symbol("a") like "foo%") && (Symbol("a") like "b%"))
    assertEqual("a not like all ('foo%', 'b%')",
      !(Symbol("a") like "foo%") && !(Symbol("a") like "b%"))
    assertEqual("not (a like all ('foo%', 'b%'))",
      !((Symbol("a") like "foo%") && (Symbol("a") like "b%")))
  ...
  test("function expressions") {
    ...
    assertEqual("grouping(distinct a, b)", Symbol("grouping")
      .distinctFunction(Symbol("a"), Symbol("b")))
  ...
  test("window function expressions") {
    ...
    assertEqual(
      ...
      WindowExpression(Symbol("sum").function(Symbol("product") + 1),
        WindowSpecDefinition(Seq(Symbol("product") / 2 + 1), Seq(Literal(2).asc),
          UnspecifiedFrame)))
  }
  ...
  test("scalar sub-query") {
    assertEqual(
      ...
      ScalarSubquery(table("tbl").select(Symbol("max")
        .function(Symbol("val")))) > Symbol("current"))
    assertEqual(
      "a = (select b from s)",
  ...
  test("case when") {
    ...
    assertEqual("case (a or b) when true then c when false then d else e end",
      CaseKeyWhen(Symbol("a") || Symbol("b"), Seq(true, Symbol("c"), false, Symbol("d"),
        Symbol("e"))))
    ...
    assertEqual("case when a = 1 then b when a = 2 then c else d end",
      CaseWhen(Seq((Symbol("a") === 1, Symbol("b").expr), (Symbol("a") === 2, Symbol("c").expr)),
        Symbol("d")))
    assertEqual("case when (1) + case when a > b then c else d end then f else g end",
      CaseWhen(Seq((Literal(1) + CaseWhen(Seq((Symbol("a") > Symbol("b"), Symbol("c").expr)),
        Symbol("d").expr), Symbol("f").expr)), Symbol("g")))
  }
  ...
  test("composed expressions") {
    ...
    assertEqual("1 - f('o', o(bar))", Literal(1) - Symbol("f")
      .function("o", Symbol("o").function(Symbol("bar"))))
