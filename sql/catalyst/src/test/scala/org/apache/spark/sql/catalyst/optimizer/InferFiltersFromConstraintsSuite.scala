...
class InferFiltersFromConstraintsSuite extends PlanTest {
  ...
  test("filter: filter out constraints in condition") {
    ...
    val correctAnswer = testRelation
      .where(IsNotNull(Symbol("a")) && IsNotNull(Symbol("b")) &&
        Symbol("a") === Symbol("b") && Symbol("a") === 1 && Symbol("b") === 1).analyze
    val optimized = Optimize.execute(originalQuery)
    comparePlans(optimized, correctAnswer)
  }
  ...
  test("single inner join: filter out values on either side on equi-join keys") {
    ...
    val originalQuery = x.join(y,
      ...
      .analyze
    val left = x.where(IsNotNull(Symbol("a")) && "x.a".attr === 1)
    val right = y.where(IsNotNull(Symbol("a")) && IsNotNull(Symbol("c")) && "y.c".attr > 5 &&
      "y.a".attr === 1)
  ...
  test("single inner join: filter out nulls on either side on non equal keys") {
    ...
    val originalQuery = x.join(y,
      ...
      .analyze
    val left = x.where(IsNotNull(Symbol("a")) && IsNotNull(Symbol("b")) && "x.b".attr === 1)
    val right = y.where(IsNotNull(Symbol("a")) && IsNotNull(Symbol("c")) && "y.c".attr > 5)
    val correctAnswer = left.join(right, condition = Some("x.a".attr =!= "y.a".attr)).analyze
    val optimized = Optimize.execute(originalQuery)
    comparePlans(optimized, correctAnswer)
  ...
  test("single inner join with pre-existing filters: filter out values on either side") {
    val x = testRelation.subquery(Symbol("x"))
    val y = testRelation.subquery(Symbol("y"))
    val originalQuery = x.where(Symbol("b") > 5).join(y.where(Symbol("a") === 10),
      condition = Some("x.a".attr === "y.a".attr && "x.b".attr === "y.b".attr)).analyze
    val left = x.where(IsNotNull(Symbol("a")) && Symbol("a") === 10 && IsNotNull(Symbol("b")) &&
      Symbol("b") > 5)
    val right = y.where(IsNotNull(Symbol("a")) && IsNotNull(Symbol("b")) && Symbol("a") === 10 &&
      Symbol("b") > 5)
  ...
  test("multiple inner joins: filter out values on all sides on equi-join keys") {
    val t1 = testRelation.subquery(Symbol("t1"))
    val t2 = testRelation.subquery(Symbol("t2"))
    val t3 = testRelation.subquery(Symbol("t3"))
    val t4 = testRelation.subquery(Symbol("t4"))

    val originalQuery = t1.where(Symbol("b") > 5)
    ...
    val correctAnswer = t1.where(IsNotNull(Symbol("b")) && Symbol("b") > 5)
      .join(t2.where(IsNotNull(Symbol("b")) && Symbol("b") > 5),
        condition = Some("t1.b".attr === "t2.b".attr))
      .join(t3.where(IsNotNull(Symbol("b")) && Symbol("b") > 5),
        condition = Some("t2.b".attr === "t3.b".attr))
      .join(t4.where(IsNotNull(Symbol("b")) && Symbol("b") > 5),
  ...
  test("inner join with filter: filter out values on all sides on equi-join keys") {
    ...
    val correctAnswer = x.where(IsNotNull(Symbol("a")) && Symbol("a").attr > 5)
      .join(y.where(IsNotNull(Symbol("a")) && Symbol("a").attr > 5), Inner,
  ...
  test("inner join with alias: alias contains multiple attributes") {
    ...
    val originalQuery = t1.select(Symbol("a"),
      Coalesce(Seq(Symbol("a"), Symbol("b"))).as(Symbol("int_col"))).as("t")
      .join(t2, Inner, Some("t.a".attr === "t2.a".attr && "t.int_col".attr === "t2.a".attr))
      .analyze
    val correctAnswer = t1
      .where(IsNotNull(Symbol("a")) && IsNotNull(Coalesce(Seq(Symbol("a"), Symbol("b")))) &&
        Symbol("a") === Coalesce(Seq(Symbol("a"), Symbol("b"))))
      .select(Symbol("a"), Coalesce(Seq(Symbol("a"), Symbol("b"))).as(Symbol("int_col"))).as("t")
      .join(t2.where(IsNotNull(Symbol("a"))), Inner,
  ...
  test("inner join with alias: alias contains single attributes") {
    ...
    val originalQuery = t1.select(Symbol("a"), Symbol("b").as(Symbol("d"))).as("t")
      .join(t2, Inner, Some("t.a".attr === "t2.a".attr && "t.d".attr === "t2.a".attr))
      .analyze
    val correctAnswer = t1
      .where(IsNotNull(Symbol("a")) && IsNotNull(Symbol("b")) &&Symbol("a") === Symbol("b"))
      .select(Symbol("a"), Symbol("b").as(Symbol("d"))).as("t")
      .join(t2.where(IsNotNull(Symbol("a"))), Inner,
  ...
  test("generate correct filters for alias that don't produce recursive constraints") {
    ...
    val originalQuery = t1.select(Symbol("a").as(Symbol("x")), Symbol("b").as(Symbol("y")))
      .where(Symbol("x") === 1 && Symbol("x") === Symbol("y")).analyze
    val correctAnswer =
      t1.where(Symbol("a") === 1 && Symbol("b") === 1 && Symbol("a") === Symbol("b") &&
        IsNotNull(Symbol("a")) && IsNotNull(Symbol("b")))
        .select(Symbol("a").as(Symbol("x")), Symbol("b").as(Symbol("y"))).analyze
    val optimized = Optimize.execute(originalQuery)
    comparePlans(optimized, correctAnswer)
  }
  ...
  test("No inferred filter when constraint propagation is disabled") {
    withSQLConf(SQLConf.CONSTRAINT_PROPAGATION_ENABLED.key -> "false") {
      val originalQuery = testRelation.where(Symbol("a") === 1 && Symbol("a") === Symbol("b"))
        .analyze
      val optimized = Optimize.execute(originalQuery)
      comparePlans(optimized, originalQuery)
    }
  ...
  test("constraints should be inferred from aliased literals") {
    val originalLeft = testRelation.subquery(Symbol("left")).as("left")
    val optimizedLeft = testRelation.subquery(Symbol("left"))
      .where(IsNotNull(Symbol("a")) && Symbol("a") <=> 2).as("left")

    val right = Project(Seq(Literal(2).as("two")),
      testRelation.subquery(Symbol("right"))).as("right")
  ...
  test("SPARK-23405: left-semi equal-join should filter out null join keys on both sides") {
    ...
    testConstraintsAfterJoin(x, y,
      x.where(IsNotNull(Symbol("a"))), y.where(IsNotNull(Symbol("a"))), LeftSemi)
  }
  ...
  test("SPARK-21479: Outer join after-join filters push down to null-supplying side") {
    ...
    val left = x.where(IsNotNull(Symbol("a")) && Symbol("a") === 2)
    val right = y.where(IsNotNull(Symbol("a")) && Symbol("a") === 2)
  ...
  test("SPARK-21479: Outer join pre-existing filters push down to null-supplying side") {
    ...
    val left = x.where(IsNotNull(Symbol("a")) && Symbol("a") > 5)
    val right = y.where(IsNotNull(Symbol("a")) && Symbol("a") > 5)
  ...
  test("SPARK-21479: Outer join no filter push down to preserved side") {
    ...
    testConstraintsAfterJoin(
      x, y.where("a".attr === 1),
      x, y.where(IsNotNull(Symbol("a")) && Symbol("a") === 1),
      LeftOuter)
  }
  ...
  test("SPARK-23564: left anti join should filter out null join keys on right side") {
    val x = testRelation.subquery(Symbol("x"))
    val y = testRelation.subquery(Symbol("y"))
    testConstraintsAfterJoin(x, y, x, y.where(IsNotNull(Symbol("a"))), LeftAnti)
  }

  test("SPARK-23564: left outer join should filter out null join keys on right side") {
    val x = testRelation.subquery(Symbol("x"))
    val y = testRelation.subquery(Symbol("y"))
    testConstraintsAfterJoin(x, y, x, y.where(IsNotNull(Symbol("a"))), LeftOuter)
  }

  test("SPARK-23564: right outer join should filter out null join keys on left side") {
    val x = testRelation.subquery(Symbol("x"))
    val y = testRelation.subquery(Symbol("y"))
    testConstraintsAfterJoin(x, y, x.where(IsNotNull(Symbol("a"))), y, RightOuter)
  }

  test("Constraints should be inferred from cast equality constraint(filter higher data type)") {
    val testRelation1 = LocalRelation(Symbol("a").int)
    val testRelation2 = LocalRelation(Symbol("b").long)
    val originalLeft = testRelation1.subquery(Symbol("left"))
    val originalRight = testRelation2.where(Symbol("b") === 1L).subquery(Symbol("right"))

    val left = testRelation1.where(IsNotNull(Symbol("a")) && Symbol("a").cast(LongType) === 1L)
      .subquery(Symbol("left"))
    val right = testRelation2.where(IsNotNull(Symbol("b")) && Symbol("b") === 1L)
    ...
    Seq(Some("left.a".attr === "right.b".attr.cast(IntegerType)),
      ...
      testConstraintsAfterJoin(
        originalLeft,
        originalRight,
        testRelation1.where(IsNotNull(Symbol("a"))).subquery(Symbol("left")),
        right,
        Inner,
        condition)
    }
  ...
  test("Constraints shouldn't be inferred from cast equality constraint(filter lower data type)") {
    val testRelation1 = LocalRelation(Symbol("a").int)
    val testRelation2 = LocalRelation(Symbol("b").long)
    val originalLeft = testRelation1.where(Symbol("a") === 1).subquery(Symbol("left"))
    val originalRight = testRelation2.subquery(Symbol("right"))

    val left = testRelation1.where(IsNotNull(Symbol("a")) && Symbol("a") === 1)
      .subquery(Symbol("left"))
    val right = testRelation2.where(IsNotNull(Symbol("b"))).subquery(Symbol("right"))
    ...
    Seq(Some("left.a".attr === "right.b".attr.cast(IntegerType)),
      ...
      testConstraintsAfterJoin(
        ...
        originalRight,
        left,
        testRelation2.where(IsNotNull(Symbol("b")) && Symbol("b").attr.cast(IntegerType) === 1)
          .subquery(Symbol("right")),
        Inner,
        condition)
    }
