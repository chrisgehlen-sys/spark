...
class PropagateEmptyRelationSuite extends PlanTest {
  ...
  val testRelation3 = LocalRelation
    .fromExternalRows(Seq(Symbol("c").int.notNull.withMetadata(metadata)), data = Seq(Row(1)))
  ...
  test("SPARK-32241: remove empty relation children from Union") {
    ...
    val correctAnswer2 = testRelation2.select(Symbol("b").as(Symbol("a"))).analyze
    ...
    val correctAnswer4 = testRelation2.union(testRelation3).select(Symbol("b").as(Symbol("a")))
      .analyze
  ...
  test("propagate empty relation through Join") {
    ...
    val testcases = Seq(
      ...
      (true, false, Inner, Some(LocalRelation(Symbol("a").int, Symbol("b").int))),
      (true, false, Cross, Some(LocalRelation(Symbol("a").int, Symbol("b").int))),
      (true, false, LeftOuter,
        Some(Project(Seq(Symbol("a"), Literal(null).cast(IntegerType).as(Symbol("b"))),
          testRelation1).analyze)),
      (true, false, RightOuter, Some(LocalRelation(Symbol("a").int, Symbol("b").int))),
      (true, false, FullOuter,
        Some(Project(Seq(Symbol("a"), Literal(null).cast(IntegerType).as(Symbol("b"))),
          testRelation1).analyze)),
      (true, false, LeftAnti, Some(testRelation1)),
      (true, false, LeftSemi, Some(LocalRelation(Symbol("a").int))),

      (false, true, Inner, Some(LocalRelation(Symbol("a").int, Symbol("b").int))),
      (false, true, Cross, Some(LocalRelation(Symbol("a").int, Symbol("b").int))),
      (false, true, LeftOuter, Some(LocalRelation(Symbol("a").int, Symbol("b").int))),
      (false, true, RightOuter,
        Some(Project(Seq(Literal(null).cast(IntegerType).as(Symbol("a")), Symbol("b")),
          testRelation2).analyze)),
      (false, true, FullOuter,
        Some(Project(Seq(Literal(null).cast(IntegerType).as(Symbol("a")), Symbol("b")),
          testRelation2).analyze)),
      (false, true, LeftAnti, Some(LocalRelation(Symbol("a").int))),
      (false, true, LeftSemi, Some(LocalRelation(Symbol("a").int))),

      (false, false, Inner, Some(LocalRelation(Symbol("a").int, Symbol("b").int))),
      (false, false, Cross, Some(LocalRelation(Symbol("a").int, Symbol("b").int))),
      (false, false, LeftOuter, Some(LocalRelation(Symbol("a").int, Symbol("b").int))),
      (false, false, RightOuter, Some(LocalRelation(Symbol("a").int, Symbol("b").int))),
      (false, false, FullOuter, Some(LocalRelation(Symbol("a").int, Symbol("b").int))),
      (false, false, LeftAnti, Some(LocalRelation(Symbol("a").int))),
      (false, false, LeftSemi, Some(LocalRelation(Symbol("a").int)))
    )
  ...
  test("propagate empty relation through Aggregate with grouping expressions") {
    val query = testRelation1
      .where(false)
      .groupBy(Symbol("a"))(Symbol("a"), (Symbol("a") + 1).as(Symbol("x")))

    val optimized = Optimize.execute(query.analyze)
    val correctAnswer = LocalRelation(Symbol("a").int, Symbol("x").int).analyze
  ...
  test("don't propagate empty relation through Aggregate without grouping expressions") {
    ...
    val optimized = Optimize.execute(query.analyze)
    val correctAnswer = LocalRelation(Symbol("a").int).groupBy()().analyze
  ...
  test("propagate empty relation keeps the plan resolved") {
    val query = testRelation1.join(
      LocalRelation(Symbol("a").int, Symbol("b").int), UsingJoin(FullOuter, "a" :: Nil), None)
    val optimized = Optimize.execute(query.analyze)
