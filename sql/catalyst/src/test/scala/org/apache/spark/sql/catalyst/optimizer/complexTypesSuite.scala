...
class ComplexTypesSuite extends PlanTest with ExpressionEvalHelper {
  ...
  private val idAtt = Symbol("id").long.notNull
  private val nullableIdAtt = Symbol("nullable_id").long

  private val relation = LocalRelation(idAtt, nullableIdAtt)
  private val testRelation = LocalRelation(Symbol("a").int, Symbol("b").int, Symbol("c").int,
    Symbol("d").double, Symbol("e").int)
  ...
  test("explicit get from namedStruct") {
    val query = relation
      .select(
        GetStructField(
          CreateNamedStruct(Seq("att", Symbol("id") )),
          0,
          None) as "outerAtt")
    val expected = relation.select(Symbol("id") as "outerAtt")
  ...
  test("explicit get from named_struct- expression maintains original deduced alias") {
    val query = relation
      .select(GetStructField(CreateNamedStruct(Seq("att", Symbol("id"))), 0, None))

    val expected = relation
      .select(Symbol("id") as "named_struct(att, id).att")
  ...
  test("collapsed getStructField ontop of namedStruct") {
    val query = relation
      .select(CreateNamedStruct(Seq("att", Symbol("id"))) as "struct1")
      .select(GetStructField(Symbol("struct1"), 0, None) as "struct1Att")
    val expected = relation.select(Symbol("id") as "struct1Att")
    checkRule(query, expected)
  }
    ...
    val query = relation
      .select(
        CreateNamedStruct(Seq(
          "att1", Symbol("id"),
          "att2", Symbol("id") * Symbol("id"))) as "struct1")
      .select(
        GetStructField(Symbol("struct1"), 0, None) as "struct1Att1",
        GetStructField(Symbol("struct1"), 1, None) as "struct1Att2")

    val expected =
      relation.
        select(
          Symbol("id") as "struct1Att1",
          (Symbol("id") * Symbol("id")) as "struct1Att2")
    ...
    val query = relation
      .select(
        CreateNamedStruct(Seq(
          ...
          "att2", Symbol("id") * Symbol("id"))) as "struct1")
      .select(
        GetStructField(Symbol("struct1"), 0, None),
        GetStructField(Symbol("struct1"), 1, None))

    val expected =
      relation.
        select(
          Symbol("id") as "struct1.att1",
          (Symbol("id") * Symbol("id")) as "struct1.att2")
  ...
  test("simplified array ops") {
    val rel = relation.select(
      CreateArray(Seq(
        CreateNamedStruct(Seq(
          "att1", Symbol("id"),
          "att2", Symbol("id") * Symbol("id"))),
        CreateNamedStruct(Seq(
          "att1", Symbol("id") + 1,
          "att2", (Symbol("id") + 1) * (Symbol("id") + 1))
       ))
      ) as "arr"
    )
    val query = rel
      .select(
        GetArrayStructFields(Symbol("arr"),
          StructField("att1", LongType, false), 0, 1, false) as "a1",
        GetArrayItem(Symbol("arr"), 1) as "a2",
        GetStructField(GetArrayItem(Symbol("arr"), 1), 0, None) as "a3",
        GetArrayItem(
          GetArrayStructFields(Symbol("arr"),
    ...
    val expected = relation
      .select(
        CreateArray(Seq(Symbol("id"), Symbol("id") + 1L)) as "a1",
        CreateNamedStruct(Seq(
          "att1", Symbol("id") + 1L,
          "att2", (Symbol("id") + 1L) * (Symbol("id") + 1L))) as "a2",
        (Symbol("id") + 1L) as "a3",
        (Symbol("id") + 1L) as "a4")
    checkRule(query, expected)
  }
  ...
  test("simplify map ops") {
    val rel = relation
      .select(
        CreateMap(Seq(
          "r1", CreateNamedStruct(Seq("att1", Symbol("id"))),
          "r2", CreateNamedStruct(Seq("att1", Symbol("id") + 1L)))) as "m")
    val query = rel
      .select(
        GetMapValue(Symbol("m"), "r1") as "a1",
        GetStructField(GetMapValue(Symbol("m"), "r1"), 0, None) as "a2",
        GetMapValue(Symbol("m"), "r32") as "a3",
        GetStructField(GetMapValue(Symbol("m"), "r32"), 0, None) as "a4")

    val expected =
      relation.select(
        CreateNamedStruct(Seq("att1", Symbol("id"))) as "a1",
        Symbol("id") as "a2",
        Literal.create(
          null,
          StructType(
  ...
  test("simplify map ops, constant lookup, dynamic keys") {
    val query = relation.select(
      GetMapValue(
        CreateMap(Seq(
          Symbol("id"), Symbol("id") + 1L,
          Symbol("id") + 1L, Symbol("id") + 2L,
          Symbol("id") + 2L, Symbol("id") + 3L,
          Literal(13L), Symbol("id"),
          Symbol("id") + 3L, Symbol("id") + 4L,
          Symbol("id") + 4L, Symbol("id") + 5L)),
        13L) as "a")

    val expected = relation
      .select(
        CaseWhen(Seq(
          (EqualTo(13L, Symbol("id")), Symbol("id") + 1L),
          (EqualTo(13L, Symbol("id") + 1L), Symbol("id") + 2L),
          (EqualTo(13L, Symbol("id") + 2L), Symbol("id") + 3L),
          (Literal(true), Symbol("id")))) as "a")
    checkRule(query, expected)
  }
  ...
  test("simplify map ops, dynamic lookup, dynamic keys, lookup is equivalent to one of the keys") {
    ...
    val expected = relation
      .select(
        CaseWhen(Seq(
          (EqualTo(Symbol("id") + 3L, Symbol("id")), Symbol("id") + 1L),
          (EqualTo(Symbol("id") + 3L, Symbol("id") + 1L), Symbol("id") + 2L),
          (EqualTo(Symbol("id") + 3L, Symbol("id") + 2L), Symbol("id") + 3L),
          (Literal(true), Symbol("id") + 4L))) as "a")
    checkRule(query, expected)
  }
  ...
  test("simplify map ops, no positive match") {
    ...
    val expected = relation.select(
      CaseWhen(Seq(
        (EqualTo(Symbol("id") + 30L, Symbol("id")), Symbol("id") + 1L),
        (EqualTo(Symbol("id") + 30L, Symbol("id") + 1L), Symbol("id") + 2L),
        (EqualTo(Symbol("id") + 30L, Symbol("id") + 2L), Symbol("id") + 3L),
        (EqualTo(Symbol("id") + 30L, Symbol("id") + 3L), Symbol("id") + 4L),
        (EqualTo(Symbol("id") + 30L, Symbol("id") + 4L), Symbol("id") + 5L))) as "a")
  ...
  test("simplify map ops, constant lookup, mixed keys, eliminated constants") {
    ...
    val expected = relation
      .select(
        CaseKeyWhen(13L,
          Seq(Symbol("id"), Symbol("id") + 1L,
            Symbol("id") + 1L, Symbol("id") + 2L,
            Symbol("id") + 2L, Symbol("id") + 3L,
            Symbol("id") + 3L, Symbol("id") + 4L,
            Symbol("id") + 4L, Symbol("id") + 5L)) as "a")
  ...
  test("simplify map ops, potential dynamic match with null value + an absolute constant match") {
      ...
      .select(
        GetMapValue(
          CreateMap(Seq(
            Symbol("id"), Symbol("id") + 1L,
            Symbol("id") + 1L, Symbol("id") + 2L,
            Symbol("id") + 2L, Literal.create(null, LongType),
            Literal(2L), Symbol("id"),
            Symbol("id") + 3L, Symbol("id") + 4L,
            Symbol("id") + 4L, Symbol("id") + 5L)),
          2L ) as "a")
    ...
    val expected = relation
      .select(
        CaseWhen(Seq(
          (EqualTo(2L, Symbol("id")), Symbol("id") + 1L),
          // these two are possible matches, we can't tell until runtime
          (EqualTo(2L, Symbol("id") + 1L), Symbol("id") + 2L),
          (EqualTo(2L, Symbol("id") + 2L), Literal.create(null, LongType)),
          ...
          (Literal.TrueLiteral, Symbol("id")))) as "a")
    checkRule(rel, expected)
  }

  test("SPARK-23500: Simplify array ops that are not at the top node") {
    val query = LocalRelation(Symbol("id").long)
      .select(
        CreateArray(Seq(
          CreateNamedStruct(Seq(
            "att1", Symbol("id"),
      ...
      .select(
        GetStructField(GetArrayItem(Symbol("arr"), 1), 0, None) as "a1",
        GetArrayItem(
          GetArrayStructFields(Symbol("arr"),
            ...
            containsNull = false),
          ordinal = 1) as "a2")
      .orderBy(Symbol("id").asc)

    val expected = LocalRelation(Symbol("id").long)
      .select(
        (Symbol("id") + 1L) as "a1",
        (Symbol("id") + 1L) as "a2")
      .orderBy(Symbol("id").asc)
  ...
  test("SPARK-23500: Simplify map ops that are not top nodes") {
    val query =
      LocalRelation(Symbol("id").long)
        .select(
          CreateMap(Seq(
            "r1", Symbol("id"),
            "r2", Symbol("id") + 1L)) as "m")
        .select(
          GetMapValue(Symbol("m"), "r1") as "a1",
          GetMapValue(Symbol("m"), "r32") as "a2")
        .orderBy(Symbol("id").asc)
        .select(Symbol("a1"), Symbol("a2"))

    val expected =
      LocalRelation(Symbol("id").long).select(
        Symbol("id") as "a1",
  ...
  test("SPARK-23500: Simplify complex ops that aren't at the plan root") {
    val structRel = relation
      .select(GetStructField(
        CreateNamedStruct(Seq("att1", Symbol("nullable_id"))), 0, None) as "foo")
      .groupBy($"foo")("1")
    val structExpected = relation
      .select(Symbol("nullable_id") as "foo")
      .groupBy($"foo")("1")
    checkRule(structRel, structExpected)

    val arrayRel = relation
      .select(GetArrayItem(
        CreateArray(Seq(Symbol("nullable_id"), Symbol("nullable_id") + 1L)), 0) as "a1")
      .groupBy($"a1")("1")
    val arrayExpected = relation.select(Symbol("nullable_id") as "a1").groupBy($"a1")("1")
    checkRule(arrayRel, arrayExpected)

    val mapRel = relation
      .select(GetMapValue(CreateMap(Seq("id", Symbol("nullable_id"))), "id") as "m1")
      .groupBy($"m1")("1")
    val mapExpected = relation
      .select(Symbol("nullable_id") as "m1")
      .groupBy($"m1")("1")
  ...
  test("SPARK-23500: Ensure that aggregation expressions are not simplified") {
    ...
    val structAggRel = relation.groupBy(
      CreateNamedStruct(Seq("att1", Symbol("nullable_id"))))(
      GetStructField(CreateNamedStruct(Seq("att1", Symbol("nullable_id"))), 0, None))
    checkRule(structAggRel, structAggRel)

    val arrayAggRel = relation.groupBy(
      CreateArray(Seq(Symbol("nullable_id"))))(GetArrayItem(
      CreateArray(Seq(Symbol("nullable_id"))), 0))
    ...
    val originalQuery = relation
      .groupBy(Symbol("id"))(
        GetMapValue(CreateMap(Seq(Symbol("id"), Symbol("id") + 1L)), 0L) as "a"
      )
  ...
  test("SPARK-23500: namedStruct and getField in the same Project #1") {
    val originalQuery =
      testRelation
        .select(
          namedStruct("col1", Symbol("b"), "col2", Symbol("c")).as("s1"), Symbol("a"), Symbol("b"))
        .select(Symbol("s1") getField "col2" as Symbol("s1Col2"),
          namedStruct("col1", Symbol("a"), "col2", Symbol("b")).as("s2"))
        .select(Symbol("s1Col2"), Symbol("s2") getField "col2" as Symbol("s2Col2"))
    val correctAnswer =
      testRelation
        .select(Symbol("c") as Symbol("s1Col2"), Symbol("b") as Symbol("s2Col2"))
  ...
  test("SPARK-23500: namedStruct and getField in the same Project #2") {
    val originalQuery =
      testRelation
        .select(
          namedStruct("col1", Symbol("b"), "col2", Symbol("c")) getField "col2" as Symbol("sCol2"),
          namedStruct("col1", Symbol("a"), "col2", Symbol("c")) getField "col1" as Symbol("sCol1"))
    val correctAnswer =
      testRelation
        .select(Symbol("c") as Symbol("sCol2"), Symbol("a") as Symbol("sCol1"))
    checkRule(originalQuery, correctAnswer)
  }
  ...
  private val structAttr = Symbol("struct1").struct(Symbol("a").int, Symbol("b").int)
    .withNullability(false)
  private val testStructRelation = LocalRelation(structAttr)

  private val nullableStructAttr = Symbol("struct1").struct(Symbol("a").int, Symbol("b").int)
  ...
  test("simplify GetStructField on basic UpdateFields") {
    def check(fieldOps: Seq[StructFieldsOperation], ordinal: Int, expected: Expression): Unit = {
      def query(relation: LocalRelation): LogicalPlan =
        relation.select(GetStructField(UpdateFields(Symbol("struct1"), fieldOps), ordinal)
          .as("res"))
      ...
      checkRule(
        ...
        testNullableStructRelation.select((expected match {
          ...
          case expr => If(IsNull(Symbol("struct1")), Literal(null, expr.dataType), expr)
        }).as("res")))
    }
    ...
    check(WithField("c", Literal(3)) :: Nil, 0, GetStructField(Symbol("struct1"), 0))
    check(WithField("c", Literal(3)) :: Nil, 1, GetStructField(Symbol("struct1"), 1))
    ...
    check(WithField("a", Literal(1)) :: Nil, 1, GetStructField(Symbol("struct1"), 1))
    check(WithField("b", Literal(2)) :: Nil, 0, GetStructField(Symbol("struct1"), 0))
    ...
    check(WithField("c", Literal(3)) :: WithField("c", Literal(4)) :: Nil, 0, GetStructField(Symbol("struct1"), 0))
    check(WithField("c", Literal(3)) :: WithField("d", Literal(4)) :: Nil, 0, GetStructField(Symbol("struct1"), 0))
    check(WithField("c", Literal(3)) :: WithField("c", Literal(4)) :: Nil, 1, GetStructField(Symbol("struct1"), 1))
    check(WithField("c", Literal(3)) :: WithField("d", Literal(4)) :: Nil, 1, GetStructField(Symbol("struct1"), 1))
    ...
    check(DropField("b") :: Nil, 0, GetStructField(Symbol("struct1"), 0))
    check(DropField("a") :: Nil, 0, GetStructField(Symbol("struct1"), 1))

    // drop attribute, add attribute, extract an attribute from the original struct
    check(DropField("b") :: WithField("c", Literal(3)) :: Nil, 0, GetStructField(Symbol("struct1"), 0))
    check(DropField("a") :: WithField("c", Literal(3)) :: Nil, 0, GetStructField(Symbol("struct1"), 1))
    ...
    check(WithField("c", Literal(3)) :: DropField("a") :: Nil, 0, GetStructField(Symbol("struct1"), 1))
    check(WithField("c", Literal(3)) :: DropField("b") :: Nil, 0, GetStructField(Symbol("struct1"), 0))
    ...
    check(WithField("b", Literal(3)) :: DropField("b") :: Nil, 0, GetStructField(Symbol("struct1"), 0))
    check(WithField("a", Literal(3)) :: DropField("a") :: Nil, 0, GetStructField(Symbol("struct1"), 1))

    // add attribute, drop same attribute, extract an attribute from the original struct
    check(WithField("c", Literal(3)) :: DropField("c") :: Nil, 0, GetStructField(Symbol("struct1"), 0))
    check(WithField("c", Literal(3)) :: DropField("c") :: Nil, 1, GetStructField(Symbol("struct1"), 1))
    ...
    check(DropField("b") :: WithField("b", Literal(3)) :: Nil, 0, GetStructField(Symbol("struct1"), 0))
    check(DropField("a") :: WithField("a", Literal(3)) :: Nil, 0, GetStructField(Symbol("struct1"), 1))
    ...
    check(DropField("c") :: WithField("c", Literal(3)) :: Nil, 0, GetStructField(Symbol("struct1"), 0))
    check(DropField("c") :: WithField("c", Literal(3)) :: Nil, 1, GetStructField(Symbol("struct1"), 1))
  ...
  test("simplify GetStructField that is extracting a field nested inside a struct") {
    val struct2 = Symbol("struct2").struct(Symbol("b").int)
    ...
    def addFieldFromSameStructAndThenExtractIt(relation: LocalRelation): LogicalPlan =
      relation.select(GetStructField(
        UpdateFields(Symbol("struct1"), WithField("b",
          GetStructField(Symbol("struct1"), 0)) :: Nil), 1).as("res"))

    checkRule(
      addFieldFromSameStructAndThenExtractIt(testStructRelation),
      testStructRelation.select(GetStructField(Symbol("struct1"), 0).as("res")))

    checkRule(
      addFieldFromSameStructAndThenExtractIt(testNullableStructRelation),
      testNullableStructRelation.select(GetStructField(Symbol("struct1"), 0).as("res")))
    ...
    def addFieldFromAnotherStructAndThenExtractIt(relation: LocalRelation): LogicalPlan =
      relation.select(GetStructField(
        UpdateFields(Symbol("struct1"), WithField("b",
          GetStructField(Symbol("struct2"), 0)) :: Nil), 1).as("res"))

    checkRule(
      addFieldFromAnotherStructAndThenExtractIt(testStructRelation),
      testStructRelation.select(GetStructField(Symbol("struct2"), 0).as("res")))

    checkRule(
      addFieldFromAnotherStructAndThenExtractIt(testNullableStructRelation),
      testNullableStructRelation.select(
        If(IsNull(Symbol("struct1")), Literal(null, IntegerType),
          GetStructField(Symbol("struct2"), 0)).as("res")))
  }
  ...
  test("simplify GetStructField on nested UpdateFields") {
    ...
    checkRule(
      ...
      testNullableStructRelation.select(
        If(IsNull(Symbol("struct1")), Literal(null, IntegerType), Literal(4)) as "res"))
  ...
  test("simplify multiple GetStructField on the same UpdateFields") {
    def query(relation: LocalRelation): LogicalPlan = relation
      .select(UpdateFields(Symbol("struct1"), WithField("b", Literal(2)) :: Nil) as "struct2")
      .select(
        GetStructField(Symbol("struct2"), 0, Some("a")) as "struct1A",
        GetStructField(Symbol("struct2"), 1, Some("b")) as "struct1B")

    checkRule(
      query(testStructRelation),
      testStructRelation.select(
        GetStructField(Symbol("struct1"), 0) as "struct1A",
    ...
    checkRule(
      ...
      testNullableStructRelation.select(
        GetStructField(Symbol("struct1"), 0) as "struct1A",
        If(IsNull(Symbol("struct1")), Literal(null, IntegerType), Literal(2)) as "struct1B"))
  }

  test("simplify multiple GetStructField on different UpdateFields") {
    def query(relation: LocalRelation): LogicalPlan = relation
      .select(
        UpdateFields(Symbol("struct1"), WithField("b", Literal(2)) :: Nil) as "struct2",
        UpdateFields(Symbol("struct1"), WithField("b", Literal(3)) :: Nil) as "struct3")
      .select(
        GetStructField(Symbol("struct2"), 0, Some("a")) as "struct2A",
        GetStructField(Symbol("struct2"), 1, Some("b")) as "struct2B",
        GetStructField(Symbol("struct3"), 0, Some("a")) as "struct3A",
        GetStructField(Symbol("struct3"), 1, Some("b")) as "struct3B")

    checkRule(
      query(testStructRelation),
      testStructRelation
        .select(
          GetStructField(Symbol("struct1"), 0) as "struct2A",
          Literal(2) as "struct2B",
          GetStructField(Symbol("struct1"), 0) as "struct3A",
    ...
    checkRule(
      ...
      testNullableStructRelation
        .select(
          GetStructField(Symbol("struct1"), 0) as "struct2A",
          If(IsNull(Symbol("struct1")), Literal(null, IntegerType), Literal(2)) as "struct2B",
          GetStructField(Symbol("struct1"), 0) as "struct3A",
          If(IsNull(Symbol("struct1")), Literal(null, IntegerType), Literal(3)) as "struct3B"))
  }
  ...
  test("simplify add multiple nested fields to non-nullable struct") {
    ...
    val structLevel2 = LocalRelation(
      Symbol("a1").struct(
        Symbol("a2").struct(Symbol("a3").int.notNull)).notNull)

    val query = {
      val addB3toA1A2 = UpdateFields(Symbol("a1"), Seq(WithField("a2",
        UpdateFields(GetStructField(Symbol("a1"), 0), Seq(WithField("b3", Literal(2)))))))
    ...
    val expected = structLevel2.select(
      UpdateFields(Symbol("a1"), Seq(
        // scalastyle:off line.size.limit
        WithField("a2", UpdateFields(GetStructField(Symbol("a1"), 0), WithField("b3", 2) :: WithField("c3", 3) :: Nil))
        // scalastyle:on line.size.limit
      )).as("a1"))
  ...
  test("simplify add multiple nested fields to nullable struct") {
    ...
    val structLevel2 = LocalRelation(
      Symbol("a1").struct(
        Symbol("a2").struct(Symbol("a3").int.notNull)))
    ...
    val expected = {
      val repeatedExpr = UpdateFields(GetStructField(Symbol("a1"), 0),
        WithField("b3", Literal(2)) :: Nil)
      ...
      structLevel2.select(
        UpdateFields(Symbol("a1"), Seq(
          WithField("a2", UpdateFields(
            If(IsNull(Symbol("a1")), Literal(null, repeatedExprDataType), repeatedExpr),
  ...
  test("simplify drop multiple nested fields in non-nullable struct") {
    ...
    val structLevel2 = LocalRelation(
      Symbol("a1").struct(
        Symbol("a2").struct(Symbol("a3").int.notNull, Symbol("b3").int.notNull,
          Symbol("c3").int.notNull).notNull
      ).notNull)

    val query = {
      val dropA1A2B = UpdateFields(Symbol("a1"), Seq(WithField("a2", UpdateFields(
        GetStructField(Symbol("a1"), 0), Seq(DropField("b3"))))))
    ...
    val expected = structLevel2.select(
      UpdateFields(Symbol("a1"), Seq(
        WithField("a2", UpdateFields(GetStructField(Symbol("a1"), 0), Seq(DropField("b3"),
          DropField("c3"))))
      )).as("a1"))
  ...
  test("simplify drop multiple nested fields in nullable struct") {
    ...
    val expected = {
      val repeatedExpr = UpdateFields(GetStructField(Symbol("a1"), 0), DropField("b3") :: Nil)
      val repeatedExprDataType = StructType(Seq(
        StructField("a3", IntegerType, nullable = false),
