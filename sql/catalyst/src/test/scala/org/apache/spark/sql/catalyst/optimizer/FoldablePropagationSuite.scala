...
class FoldablePropagationSuite extends PlanTest {
  ...
  test("Propagate to groupBy clause") {
    ...
    val correctAnswer = testRelation
      .select(Symbol("a").as(Symbol("x")), Year(CurrentDate()).as(Symbol("y")), Symbol("b"))
      .groupBy(Symbol("x"), Year(CurrentDate()).as(Symbol("y")),
        Symbol("b"))(sum(Symbol("x")), avg(Year(CurrentDate())).as(Symbol("AVG")),
  ...
  test("Propagate in a complex query") {
    ...
    val correctAnswer = testRelation
      ...
      .groupBy(Symbol("x"), Year(CurrentDate()).as("y"),
        Symbol("b"))(sum(Symbol("x")), avg(Year(CurrentDate())).as(Symbol("AVG")),
        count(Symbol("b")))
  ...
  test("Propagate in subqueries of Union queries") {
    val query = Union(
      Seq(
        testRelation.select(Literal(1).as(Symbol("x")), Symbol("a"))
          .select(Symbol("x"), Symbol("x") + Symbol("a")),
    ...
    val correctAnswer = Union(
      Seq(
        testRelation.select(Literal(1).as(Symbol("x")), Symbol("a"))
          .select(Literal(1).as(Symbol("x")), (Literal(1).as(Symbol("x")) + Symbol("a"))
            .as("(x + a)")),
        testRelation.select(Literal(2).as(Symbol("x")), Symbol("a"))
          .select(Literal(2).as(Symbol("x")), (Literal(2).as(Symbol("x")) + Symbol("a"))
            .as("(x + a)"))))
  ...
  test("SPARK-32635: Replace references with foldables coming only from the node's children") {
    ...
    val right = LocalRelation(Symbol("b").int)
      .select(Symbol("b"), rightExpression).select(Symbol("b"))
    val join = left.join(right, joinType = LeftOuter, condition = Some(Symbol("b") === Symbol("a")))
