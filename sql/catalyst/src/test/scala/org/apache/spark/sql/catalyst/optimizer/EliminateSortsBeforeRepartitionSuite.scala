...
class EliminateSortsBeforeRepartitionSuite extends PlanTest {
  ...
  test("sortBy") {
    val plan = testRelation.select(Symbol("a"), Symbol("b"))
      .sortBy(Symbol("a").asc, Symbol("b").desc)
  ...
  test("orderBy") {
    val plan = testRelation.select(Symbol("a"), Symbol("b"))
      .orderBy(Symbol("a").asc, Symbol("b").asc)
    val optimizedPlan = testRelation.select(Symbol("a"), Symbol("b"))
    checkRepartitionCases(plan, optimizedPlan)
  }
  ...
  test("additional filter, distribute and sortBy") {
    val plan = testRelation.sortBy(Symbol("a").asc, Symbol("b").asc).distribute(Symbol("a"))(2)
      .where(Symbol("a") === 10)
