...
class ColumnPruningSuite extends PlanTest {
  ...
  test("Another fill Generate.unrequiredChildIndex if possible") {
    val input = LocalRelation(Symbol("a").int, Symbol("b").int,
  ...
  test("Nested column pruning for Generate") {
    ...
    runTest(
      ...
      aliases => Seq(Symbol("c").getField("d").as(aliases(0)),
        Symbol("c").getField("e").as(aliases(1))),
      Seq(2),
      Seq("explode")
    )
    ...
    runTest(
      ...
      aliases => Seq(Symbol("c").getField("d").as(aliases(0)),
        Symbol("c").getField("h").as(aliases(1))),
      Seq(2),
      Seq("h1", "h2")
    )
  }
  ...
  test("Column pruning for Project on Sort") {
    ...
    val correctAnswer = input.select(Symbol("a"), Symbol("b")).orderBy(Symbol("b").asc)
  ...
  test("Column pruning on Window with useless aggregate functions") {
    ...
    val originalQuery = input
      .groupBy(Symbol("a"), Symbol("c"), Symbol("d"))(Symbol("a"), Symbol("c"), Symbol("d"),
        winExpr.as(Symbol("window")))
      .select(Symbol("a"), Symbol("c"))
    val correctAnswer = input.select(Symbol("a"), Symbol("c"), Symbol("d"))
      .groupBy(Symbol("a"), Symbol("c"), Symbol("d"))(Symbol("a"), Symbol("c")).analyze
    val optimized = Optimize.execute(originalQuery.analyze)
  ...
  test("Column pruning on Window with selected agg expressions") {
    ...
    val originalQuery =
      input.select(Symbol("a"), Symbol("b"), Symbol("c"), Symbol("d"), winExpr.as(Symbol("window")))
        .where(Symbol("window") > 1).select(Symbol("a"), Symbol("c"))
    val correctAnswer =
  ...
  test("Column pruning on Window in select") {
    ...
    val originalQuery = input
      .select(Symbol("a"), Symbol("b"), Symbol("c"), Symbol("d"), winExpr.as(Symbol("window")))
      .select(Symbol("a"), Symbol("c"))
  ...
  test("Column pruning on Union") {
    ...
    val expected = Union(Project(Symbol("b") :: Nil, input1) ::
      Project(Symbol("d") :: Nil, input2) :: Nil).analyze
    comparePlans(Optimize.execute(query), expected)
  }
  ...
  test("SPARK-24696 ColumnPruning rule fails to remove extra Project") {
    ...
    val optimized = Optimize.execute(query)
    val expected = input.where(rand(0L) > 0.5).where(Symbol("key") < 10)
      .select(Symbol("key")).analyze
    comparePlans(optimized, expected)
  }
