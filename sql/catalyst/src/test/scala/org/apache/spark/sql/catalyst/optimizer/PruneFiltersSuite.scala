...
class PruneFiltersSuite extends PlanTest {
  ...
  test("Constraints of unionall") {
    ...
    val query =
      tr1.where(Symbol("a").attr > 10)
        .union(tr2.where(Symbol("d").attr > 10)
        .union(tr3.where(Symbol("g").attr > 10)))
    val queryWithUselessFilter = query.where(Symbol("a").attr > 10)
  ...
  test("Partial pruning") {
    ...
    val query = tr1
      .where("tr1.a".attr > 10)
      .join(tr2.where(Symbol("d").attr < 100), Inner, Some("tr1.a".attr === "tr2.d".attr))
    val queryWithExtraFilters =
      query.where("tr1.a".attr > 10 && Symbol("d").attr < 100 && "tr1.a".attr === "tr2.a".attr)

    val optimized = Optimize.execute(queryWithExtraFilters.analyze)
    val correctAnswer = tr1
      .where("tr1.a".attr > 10)
      .join(tr2.where(Symbol("d").attr < 100),
        Inner,
  ...
  test("Nondeterministic predicate is not pruned") {
    val originalQuery = testRelation.where(Rand(10) > 5).select(Symbol("a")).where(Rand(10) > 5)
      .analyze
    val optimized = Optimize.execute(originalQuery)
    val correctAnswer = testRelation.where(Rand(10) > 5).where(Rand(10) > 5).select(Symbol("a"))
      .analyze
    comparePlans(optimized, correctAnswer)
  }
  ...
  test("No pruning when constraint propagation is disabled") {
    ...
    withSQLConf(SQLConf.CONSTRAINT_PROPAGATION_ENABLED.key -> "false") {
      ...
      val correctAnswer = tr1
        ...
        .join(tr2.where(Symbol("d").attr < 100).where(Symbol("d").attr < 100),
