...
class OuterJoinEliminationSuite extends PlanTest {
  ...
  val testRelation1 = LocalRelation(Symbol("d").int, Symbol("e").int, Symbol("f").int)
  ...
  test("joins: full outer to left") {
    ...
    val optimized = Optimize.execute(originalQuery.analyze)
    val left = testRelation.where(Symbol("a") <=> 2)
  ...
  test("joins: right to inner") {
    ...
    val optimized = Optimize.execute(originalQuery.analyze)
    val left = testRelation.where(Symbol("b") > 2)
    val right = testRelation1
    val correctAnswer =
      left.join(right, Inner, Option("a".attr === "d".attr)).analyze
  ...
  test("joins: left to inner") {
    ...
    val optimized = Optimize.execute(originalQuery.analyze)
    val left = testRelation
    val right = testRelation1.where(Symbol("e").isNotNull)
    val correctAnswer =
      left.join(right, Inner, Option("a".attr === "d".attr)).analyze
  ...
  test("joins: left to inner with complicated filter predicates #1") {
    ...
    val originalQuery =
      x.join(y, LeftOuter, Option("x.a".attr === "y.d".attr))
        .where(!Symbol("e").isNull || (Symbol("d").isNotNull && Symbol("f").isNull))
  ...
  test("joins: left to inner with complicated filter predicates #2") {
    ...
    val originalQuery =
      x.join(y, LeftOuter, Option("x.a".attr === "y.d".attr))
        .where(Symbol("e").in(1, 2))

    val optimized = Optimize.execute(originalQuery.analyze)
    val left = testRelation
    val right = testRelation1.where(Symbol("e").in(1, 2))
    val correctAnswer =
      left.join(right, Inner, Option("a".attr === "d".attr)).analyze
