...
class AggregateOptimizeSuite extends AnalysisTest {
  ...
  test("remove literals in grouping expression") {
    val query = testRelation.groupBy(Symbol("a"), Literal("1"),
      Literal(1) + Literal(2))(sum(Symbol("b")))
  ...
  test("Remove aliased literals") {
    val query = testRelation.select(Symbol("a"), Symbol("b"), Literal(1).as(Symbol("y")))
      .groupBy(Symbol("a"), Symbol("y"))(sum(Symbol("b")))
    val optimized = Optimize.execute(analyzer.execute(query))
    val correctAnswer = testRelation.select(Symbol("a"), Symbol("b"), Literal(1).as(Symbol("y")))
      .groupBy(Symbol("a"))(sum(Symbol("b"))).analyze

    comparePlans(optimized, correctAnswer)
  }

  test("remove repetition in grouping expression") {
    val query = testRelation.groupBy(Symbol("a") + 1, Symbol("b") + 2,
      Literal(1) + Symbol("A"), Literal(2) + Symbol("B"))(sum(Symbol("c")))
    val optimized = Optimize.execute(analyzer.execute(query))
    val correctAnswer = testRelation
      .groupBy(Symbol("a") + 1, Symbol("b") + 2)(sum(Symbol("c"))).analyze
