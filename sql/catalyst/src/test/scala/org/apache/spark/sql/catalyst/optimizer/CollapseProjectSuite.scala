...
class CollapseProjectSuite extends PlanTest {
  ...
  test("collapse two deterministic, independent projects into one") {
    ...
    val correctAnswer = testRelation
      .select((Symbol("a") + 1).as(Symbol("a_plus_1")), (Symbol("b") + 1).as(Symbol("b_plus_1")))
      .analyze
  ...
  test("collapse two deterministic, dependent projects into one") {
    ...
    val correctAnswer = testRelation
      .select(((Symbol("a") + 1).as(Symbol("a_plus_1")) + 1).as(Symbol("a_plus_2")), Symbol("b"))
      .analyze
  ...
  test("collapse redundant alias through limit") {
    ...
    val query = relation.select(Symbol("a") as Symbol("b")).limit(1)
      .select(Symbol("b") as Symbol("c")).analyze
  ...
  test("collapse redundant alias through local limit") {
    val relation = LocalRelation(Symbol("a").int, Symbol("b").int)
    val query = LocalLimit(1, relation.select(Symbol("a") as Symbol("b")))
      .select(Symbol("b") as Symbol("c")).analyze
    val optimized = Optimize.execute(query)
  ...
  test("collapse redundant alias through repartition") {
    val relation = LocalRelation(Symbol("a").int, Symbol("b").int)
    val query = relation.select(Symbol("a") as Symbol("b")).repartition(1)
      .select(Symbol("b") as Symbol("c")).analyze
    val optimized = Optimize.execute(query)
  ...
  test("collapse redundant alias through sample") {
    val relation = LocalRelation(Symbol("a").int, Symbol("b").int)
    val query = Sample(0.0, 0.6, false, 11L, relation.select(Symbol("a") as Symbol("b")))
      .select(Symbol("b") as Symbol("c")).analyze
    val optimized = Optimize.execute(query)
