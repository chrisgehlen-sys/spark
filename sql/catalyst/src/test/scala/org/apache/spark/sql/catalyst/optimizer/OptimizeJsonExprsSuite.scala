...
class OptimizeJsonExprsSuite extends PlanTest with ExpressionEvalHelper {
  ...
  private val structAtt = Symbol("struct").struct(schema).notNull
  private val jsonAttr = Symbol("json").string
  ...
  test("SPARK-32948: optimize from_json + to_json") {
    ...
    val query1 = testRelation
      .select(JsonToStructs(schema, options, StructsToJson(options, Symbol("struct"))).as("struct"))
    val optimized1 = Optimizer.execute(query1.analyze)

    val expected = testRelation.select(Symbol("struct").as("struct")).analyze
      ...
      .select(
        JsonToStructs(schema, options,
          StructsToJson(options,
            JsonToStructs(schema, options,
              StructsToJson(options, Symbol("struct"))))).as("struct"))
  ...
  test("SPARK-32948: not optimize from_json + to_json if schema is different") {
    ...
    val expected = testRelation.select(
      JsonToStructs(schema, options, StructsToJson(options, Symbol("struct"))).as("struct")).analyze
  ...
  test("SPARK-32948: if user gives schema with different letter case under case-insensitive") {
    withSQLConf(SQLConf.CASE_SENSITIVE.key -> "false") {
      ...
      val query = testRelation
        .select(JsonToStructs(schema, options, StructsToJson(options, Symbol("struct")))
          .as("struct"))
      val optimized = Optimizer.execute(query.analyze)

      val expected = testRelation.select(
        JsonToStructs(schema, options, StructsToJson(options, Symbol("struct")))
          .as("struct")).analyze
      comparePlans(optimized, expected)
    }
  }
  ...
  test("SPARK-32948: not optimize from_json + to_json if nullability is different") {
    ...
    val structAtt = Symbol("struct").struct(nonNullSchema).notNull
  ...
  test("SPARK-32948: not optimize from_json + to_json if timezone is different") {
    ...
    val query1 = testRelation
      .select(JsonToStructs(schema, options,
        StructsToJson(options, Symbol("struct"), Option(PST.getId)), UTC_OPT).as("struct"))
    val optimized1 = Optimizer.execute(query1.analyze)

    val expected1 = testRelation.select(
      JsonToStructs(schema, options,
        StructsToJson(options, Symbol("struct"), Option(PST.getId)), UTC_OPT).as("struct")).analyze
    comparePlans(optimized1, expected1)

    val query2 = testRelation
      .select(JsonToStructs(schema, options,
        StructsToJson(options, Symbol("struct"), UTC_OPT), UTC_OPT).as("struct"))
    val optimized2 = Optimizer.execute(query2.analyze)
    val expected2 = testRelation.select(Symbol("struct").as("struct")).analyze
  ...
  test("SPARK-32958: prune unnecessary columns from GetStructField + from_json") {
    ...
    val query1 = testRelation2
      .select(GetStructField(JsonToStructs(schema, options, Symbol("json")), 0))
    val optimized1 = Optimizer.execute(query1.analyze)

    val prunedSchema1 = StructType.fromDDL("a int")
    val expected1 = testRelation2
      .select(GetStructField(JsonToStructs(prunedSchema1, options, Symbol("json")), 0)).analyze
    comparePlans(optimized1, expected1)

    val query2 = testRelation2
      .select(GetStructField(JsonToStructs(schema, options, Symbol("json")), 1))
    val optimized2 = Optimizer.execute(query2.analyze)

    val prunedSchema2 = StructType.fromDDL("b int")
    val expected2 = testRelation2
      .select(GetStructField(JsonToStructs(prunedSchema2, options, Symbol("json")), 0)).analyze
    comparePlans(optimized2, expected2)
  }
  ...
  test("SPARK-32958: prune unnecessary columns from GetArrayStructFields + from_json") {
    ...
    val query1 = testRelation2
      .select(GetArrayStructFields(
        JsonToStructs(schema1, options, Symbol("json")), field1, 0, 2, true).as("a"))
    val optimized1 = Optimizer.execute(query1.analyze)

    val prunedSchema1 = ArrayType(StructType.fromDDL("a int"), containsNull = true)
    val expected1 = testRelation2
      .select(GetArrayStructFields(
        JsonToStructs(prunedSchema1, options, Symbol("json")), field1, 0, 1, true).as("a")).analyze
    comparePlans(optimized1, expected1)
    ...
    val query2 = testRelation2
      .select(GetArrayStructFields(
        JsonToStructs(schema2, options, Symbol("json")), field2, 1, 2, false).as("b"))
    val optimized2 = Optimizer.execute(query2.analyze)
    ...
    val expected2 = testRelation2
      .select(GetArrayStructFields(
        JsonToStructs(prunedSchema2, options, Symbol("json")), field2, 0, 1, false).as("b")).analyze
    comparePlans(optimized2, expected2)
  }
  ...
  test("SPARK-33007: simplify named_struct + from_json") {
    ...
    val UTC_OPT = Option("UTC")
    val json: BoundReference = Symbol("json").string.canBeNull.at(0)
