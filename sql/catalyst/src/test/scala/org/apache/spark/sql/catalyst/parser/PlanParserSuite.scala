...
class PlanParserSuite extends AnalysisTest {
  ...
  test("simple select query") {
    ...
    assertEqual("select a, b from db.c where x < 1", table("db", "c")
      .where(Symbol("x") < 1).select(Symbol("a"), Symbol("b")))
    assertEqual(
  ...
  test("aggregation") {
    ...
    // Normal
    assertEqual(sql, table("d").groupBy(Symbol("a"), Symbol("b"))(Symbol("a"), Symbol("b"),
      Symbol("sum").function(Symbol("c")).as("c")))

    // Cube
    assertEqual(s"$sql with cube",
      table("d").groupBy(Cube(Seq(Symbol("a"), Symbol("b"))))(Symbol("a"), Symbol("b"),
        Symbol("sum").function(Symbol("c")).as("c")))

    // Rollup
    assertEqual(s"$sql with rollup",
      table("d").groupBy(Rollup(Seq(Symbol("a"), Symbol("b"))))(Symbol("a"), Symbol("b"),
        Symbol("sum").function(Symbol("c")).as("c")))

    // Grouping Sets
    assertEqual(s"$sql grouping sets((a, b), (a), ())",
      GroupingSets(Seq(Seq(Symbol("a"), Symbol("b")), Seq(Symbol("a")), Seq()), Seq(Symbol("a"),
        Symbol("b")), table("d"), Seq(Symbol("a"), Symbol("b"),
        Symbol("sum").function(Symbol("c")).as("c"))))
  ...
  test("joins") {
    ...
    assertEqual(
      ...
      table("t1")
        .join(table("t2")
          .join(table("t3"), Inner, Option(Symbol("col3") === Symbol("col2"))), Inner,
          Option(Symbol("col3") === Symbol("col1")))
        .select(star()))
    assertEqual(
      "select * from t1 inner join (t2 inner join t3) on col3 = col2",
      table("t1")
        .join(table("t2").join(table("t3"), Inner, None), Inner,
          Option(Symbol("col3") === Symbol("col2")))
        .select(star()))
    assertEqual(
      "select * from t1 inner join (t2 inner join t3 on col3 = col2)",
      table("t1")
        .join(table("t2").join(table("t3"), Inner,
          Option(Symbol("col3") === Symbol("col2"))), Inner, None)
        .select(star()))
  ...
  test("scalar sub-query") {
    assertEqual(
      ...
      table("t").select(ScalarSubquery(table("s").select(Symbol("max")
        .function(Symbol("b")))).as("ss")))
    assertEqual(
      "select * from t where a = (select b from s)",
      table("t").where(Symbol("a") === ScalarSubquery(table("s").select(Symbol("b"))))
        .select(star()))
    assertEqual(
      "select g from t group by g having a > (select b from s)",
      table("t")
        .having(Symbol("g"))(Symbol("g"))(Symbol("a") > ScalarSubquery(table("s")
          .select(Symbol("b")))))
  }
