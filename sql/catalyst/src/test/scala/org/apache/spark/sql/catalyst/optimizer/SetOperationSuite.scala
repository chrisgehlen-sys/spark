...
class SetOperationSuite extends PlanTest {
  ...
  test("EXCEPT ALL rewrite") {
    ...
    val planFragment = testRelation
      ...
      .groupBy(Symbol("a"), Symbol("b"), Symbol("c"))(Symbol("a"), Symbol("b"), Symbol("c"),
        sum(Symbol("vcol")).as("sum"))
  ...
  test("INTERSECT ALL rewrite") {
    ...
    val leftRelation = testRelation
      .select(Literal(true).as("vcol1"), Literal(null, BooleanType).as("vcol2"),
        Symbol("a"), Symbol("b"), Symbol("c"))
    val rightRelation = testRelation2
      .select(Literal(null, BooleanType).as("vcol1"), Literal(true).as("vcol2"),
    ...
    val planFragment = leftRelation.union(rightRelation)
      ...
      .select(Symbol("a"), Symbol("b"), Symbol("c"),
        If(GreaterThan(Symbol("vcol1_count"), Symbol("vcol2_count")), Symbol("vcol2_count"),
          Symbol("vcol1_count")).as("min_count"))
      .analyze
    val multiplerAttr = planFragment.output.last
    val output = planFragment.output.dropRight(1)
    val expectedPlan = Project(output,
