...
class DAGSchedulerSuite extends SparkFunSuite with TempLocalSparkContext with TimeLimits {
  ...
  test("test merge 2 resource profiles multiple configs") {
    ...
    assert(mergedRp.executorResources(GPU).amount == 1)
    assert(mergedRp.executorResources(GPU).discoveryScript == "disc")
    assert(mergedRp.taskResources(GPU).amount == 1)
    ...
    assert(mergedRp.executorResources(GPU).discoveryScript == "disc")
    assert(mergedRp.taskResources(GPU).amount == 1)
    assert(mergedRp.executorResources(FPGA).amount == 2)
    assert(mergedRp.executorResources(FPGA).discoveryScript == "fdisc")
    assert(mergedRp.taskResources(FPGA).amount == 1)
    assert(mergedRp.executorResources(ResourceProfile.MEMORY).amount == 3072)
    assert(mergedRp.executorResources(ResourceProfile.PYSPARK_MEM).amount == 2048)
    assert(mergedRp.executorResources(ResourceProfile.OVERHEAD_MEM).amount == 1024)
    assert(mergedRp.executorResources(ResourceProfile.OFFHEAP_MEM).amount == 4096)
    ...
    assert(mergedRp.executorResources(GPU).amount == 4)
    assert(mergedRp.executorResources(GPU).discoveryScript == "disc")
    assert(mergedRp.taskResources(GPU).amount == 2)
  }
